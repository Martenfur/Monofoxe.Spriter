...
namespace Monofoxe.Spriter.Helpers
{
	internal static class SpriterHelper
	{
		...
		public static bool WillItBlend(SpriterMainlineKey firstKey, SpriterMainlineKey secondKey)
		{
			...
			{
				if (secondKey.BoneRefs == null || firstKey.BoneRefs.Length != secondKey.BoneRefs.Length)
				{
					return false;
				}
			}
			...
			{
				if (secondKey.ObjectRefs == null || firstKey.ObjectRefs.Length != secondKey.ObjectRefs.Length)
				{
					return false;
				}
			}
		...
		{
			var px = parent.ScaleX * child.X;
			var py = parent.ScaleY * child.Y;
			var angleRad = parent.Angle * Math.PI / 180;
		...
		{
			var nextTime = GetNextTime(keyA, keyB, animationLength);
		...
		public static float GetFactor(SpriterKey keyA, SpriterKey keyB, float animationLength, float targetTime)
		{
			...
			// This is a fix for incorrect interpolation.
			// Sometimes when targetTime is very close to animationLength,
			// other parts of the code think animation has already looped around 
			// and supply 0th and 1st keyframes instead of correct ones.
			// Since targetTime is still at the very end of the animation,
			// this leads to overshooting and completely screwing up the interpolation.
			// NOTE: This does not handle animations which go backwards.
			// If you run into any problems, try handling negative animation speed here.
			if (
				targetTime > timeB 
				&& timeB != 0 // only trigger during transitions
			)
			{
